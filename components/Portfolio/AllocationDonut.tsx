  ...
  const allocation = useMemo(() => {
    ...
    const data: SectorAllocation[] = Object.entries(sectorMap)
      .map(([name, value]) => ({
        name,
        value,
        color: SECTOR_COLORS[name] || '#929cc4',
      }))
      .sort((a, b) => b.value - a.value);

    const legend = data.map((item) => ({
      ...item,
      pct: ((item.value / stockTotal) * 100).toFixed(1) + '%',
    }));

    const heldCount = Object.values(stocks).filter((s) => s.shares > 0).length;
